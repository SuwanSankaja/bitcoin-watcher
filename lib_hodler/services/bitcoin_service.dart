...
class BitcoinService {
  ...
  /// Fetch the current BTC price and enriched hodler signal
    ...
    try {
      ...
      return {
        ...
        'signal': response['signal'] != null
            ? Signal.fromJson(response['signal'])
            : null,
      };
    } catch (e) {
  ...
  /// Fetch BTC portfolio summary (hodler accumulation metrics)
  Future<PortfolioSummary> getPortfolio() async {
    try {
      final response = await _apiClient.get(ApiConfig.portfolioEndpoint);
      return PortfolioSummary.fromJson(response);
    } catch (e) {
      throw ApiException('Failed to fetch portfolio: $e');
    }
  }
  ...
  Future<void> updateSettings(AppSettings settings) async {
    try {
      await _apiClient.post(ApiConfig.settingsEndpoint, settings.toJson());
    } catch (e) {
